...
class FollowerDebugLogger(Node):
    ...
    def _tick_fn(self):
        ...
        # ── Distance ──
        if dist >= 0:
            dc = G if dist < 20 else (Y if dist < 50 else R)
            print(f"\n  Dist to leader: {dc}{B}{dist:6.1f}m{X}  Offset: [{self._offset_n},{self._offset_e},{self._offset_d}] NED")
        else:
            print(f"\n  Dist to leader: {R}unknown{X}")

        print(f"\n  {D}Ctrl+C to stop | Log: {self._log_file or 'disabled'} | Hz: {self._hz}{X}")
