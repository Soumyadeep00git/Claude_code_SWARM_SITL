"""Swarm Bridge Node — fool-proof RFD900x + WiFi GCS bridge.

Peer-to-peer link (RFD900x serial, MAVLink2):
  TX at 10 Hz: GLOBAL_POSITION_INT (ID 33) + NAMED_VALUE_INT/gmode
  TX at  1 Hz: HEARTBEAT (FC state) + NAMED_VALUE_FLOAT (battery x3)
  RX: GLOBAL_POSITION_INT, NAMED_VALUE_INT, NAMED_VALUE_FLOAT, HEARTBEAT
  RX (from modem): RADIO_STATUS (ID 109) — link quality metrics

GCS link (WiFi/Ethernet UDP):
...
Threads:
  1. Radio TX  — GLOBAL_POSITION_INT+gmode @ 10 Hz, HB+battery @ 1 Hz
  2. Radio RX  — peer state + RADIO_STATUS
...
class SwarmBridgeNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            ...
            f"radio={radio_device}@{radio_baud} "
        ...
        # ── pymavlink serial connection to RFD900x ────────────────
        self._radio = None
